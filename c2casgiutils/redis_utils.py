...
def _init() -> None:
    ...
    redis_options_ = settings.REDIS_OPTIONS
    ...
    if sentinels:
        ...
        service_name = settings.REDIS_SERVICENAME
        if not service_name:
            message = "REDIS_SERVICENAME must be set when using Redis sentinels"
    ...
    if url:
        ...
        _LOG.info("No Redis configuration found, use REDIS_URL or REDIS_SENTINELS settings to configure it")
